...
import classes from './ImmigrationCalculator.module.css';

const ImmigrationCalculator = () => {
    let salaryWarning = '';
    ...
    const activityChangeHandler =(event) => {
        ...
        console.log('[Changed]' , event.target.textContent);
    }
    ...
    if (salary && (parseInt(salary.replace(/,/g, '')) < 3000000)) {
        salaryWarning = <Alert severity="error" elevation={6} variant="filled" >Minimum 3,000,000</Alert>;
    }
    ...
    return (
        ...
        <>
        <div className={[classes.PointsHeader, navBackground ? classes.PointsHeaderStick : ''].join(' ')} >
            <h1>{ text }Total Points: {totalPoints}</h1>
        </div>
            <div className={classes.ImmigrationCalculator}>
                <CustomSelect id='activity' items={items} selected={activity} changed={activityChangeHandler} />
                <br />
...
export default ImmigrationCalculator;
